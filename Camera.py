...
token = 'XIS2MN33MTl6AG04x42bk3gwNTlRVhHzzckQw5BqPum'
...
class Camera(BaseCamera):
    ...
    def moveDetect(img):
        ...
        #顔認証で感知しなければ動体検知画像を削除
        if Camera.faceDetect(img) is None:
            os.remove(filename)
    ...
    def faceDetect(img):
        ...
        for(x,y,w,h) in faces:
            ...
            #画像をファイルに保存
            filename=pictpath+"/cat_face_"+nowstr+".jpg"
            cv2.imwrite(filename, img)

            #ログ出力
            print(nowstr+' 顔検知 '+filename)
            time.sleep(3)
            ...
            payload = {'message': '生存を確認(ΦωΦ)！！！'}  # 送信メッセージ
            ...
            print(res)
            print(nowstr+' サーバ通知 '+filename)
            
            
            #配信画像に枠と名前を付ける
            cv2.rectangle(img, (x,y), (x+w,y+h), (0,255,0), 2)
            cv2.putText(img, str(filename), (x+5,y-5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
